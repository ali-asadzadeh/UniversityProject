...
<section>
    <div class="grid place-content-center h-screen bg-gray-50">
        ...
        <div class="bg-white border rounded-lg block w-96 mx-auto">
            ...
            @endif
            <form class="space-y-6 px-5 py-3" action="{{ route('password.update') }}" method="POST">
                @csrf
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <input id="email" name="email" type="email" value="{{ $email ?? old('email') }}" required class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:outline-none focus:ring-[#4A9C7E] sm:text-sm sm:leading-6">
                    </div>
                </div>

                <div>
                    <label for="password" class="block text-sm font-medium leading-6 text-gray-900">رمز عبور جدید:</label>
                    <div class="mt-2">
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium leading-6 text-gray-900">تأیید رمز عبور:</label>
                    <div class="mt-2">
                ...
                <div class="">
                    <button type="submit" class="flex w-full justify-center rounded-md px-3 py-1.5 text-sm font-semibold leading-6 shadow-sm text-white bg-[#5CAF90] hover:bg-[#4A9C7E] focus:ring-4 focus:outline-none focus:ring-[#4A9C7E]">بازنشانی رمز عبور</button>
                </div>
            </form>
